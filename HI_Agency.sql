select 
AgencyNo= No
, AgencyName=upper(Name)
, HubPartner = 'NTFB'--, HubAssignedNo = null
, [Address]= upper([Address])
, City= upper(City),[State]
, [ZIP Code]
, [FBC County Code]=upper([FBC County Code])
, Latitude= case 
	when Latitude in ('') then null 
	else Latitude end, 
Longitude = case 
	when  Longitude in ('') 
	then null else  Longitude end
, [UNC Activity Status]
, [FBC Agency Category Code]
, [FBC Agency Category Description]
, [Global Dimension 1 Code]
, [Global Dimension 2 Code]
, CategoryColor = case   
	when [Global Dimension 1 Code] in ('1200') then 'KIDS'
...
, HubPartner, case 
	when HubPartner = 'SL' then 'SHARING LIFE' 
	WHEN HubPartner = 'CCS' THEN 'CROSSROADS COMMUNITY SERVICES' 
	END
